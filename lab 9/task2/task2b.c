...
void detect_virus(char *buffer, unsigned int size){
    ...
    while(curr != NULL){
        for (int i = 0; i <= (size - curr->vir->SigSize) ; i++){
            // if (strcmp(curr->vir->virusName, "Flame2") == 0){
            //     printf("Starting byte: %d\nVirus name: %s\nVirus size: %d\n%x %x, %d\n", i, curr->vir->virusName, curr->vir->SigSize,buffer[i], buffer[i+curr->vir->SigSize-1], memcmp(&buffer[i], curr->vir->sig,curr->vir->SigSize));
            // }
