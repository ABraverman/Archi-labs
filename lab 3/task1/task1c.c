...
void detectVirusShell(){
    ...
    printf("Please enter an suspected file name: ");
    fgets(fileName, 100, stdin);
    sscanf(fileName, "%s", fileName);
    file = fopen(fileName,"r");
    if (file == NULL) {
...
int main(int argc, char **argv) {
    ...
    struct fun_desc menu[] = { { "Load signatures", loadSigs }, { "Print signatures", printSigs },{ "Detect viruses", detectVirusShell }, { "Quit", quit }, { NULL, NULL } };
    char tmpStr[100];
    int opNum = -1;
    int bound = 0;
